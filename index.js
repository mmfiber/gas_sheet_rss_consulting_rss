...
const CONSULTING_CONTENT_RULE = SpreadsheetApp.newDataValidation().requireValueInList(CONSULTING_CONTENT).build();
...
function setOnEdit() {
  ScriptApp.newTrigger("onEditRss")
  .forSpreadsheet(ssRss)
  .onEdit()
  .create();
}
...
function onEditRss(e) {
  ...
  if(rowIdx < CHECK_RANGE.from.rowIdx || colIdx < CHECK_RANGE.from.colIdx) return

  const sheetRss = e.source.getActiveSheet();
  const sheetStudent = ssStudent.getSheetByName(sheetRss.getSheetName());

  const sectionNum = Math.floor((rowIdx - (LABELS_TT_ROW.length + OFFSET_ROW) - 1) / LABELS_INPUT_ROW_RSS.length) + 1 // define a set of LABELS_INPUT_ROW_** as section
  const initRowIdxRss = (sectionNum - 1) * LABELS_INPUT_ROW_RSS.length + LABELS_TT_ROW.length + OFFSET_ROW + 1;
...
function isAllValuesSet(sheet, initRowIdxStudent, initColIdx) {
  let isSet = true
  const values = sheet.getRange(initRowIdxStudent, initColIdx, LABELS_INPUT_ROW_RSS.length, 1).getValues()
  values.forEach(v => {
...
function recreateSheets(){
  ...
  INDUSTRIES.forEach(name => {
    //delete sheets
    if(sheetNamesRss.some(s => s === name)) ssRss.deleteSheet(ssRss.getSheetByName(name))
    if(sheetNamesStudent.some(s => s === name)) ssStudent.deleteSheet(ssStudent.getSheetByName(name))
    //recreate sheets
...
function setSheetCommon(sheet, initColumn, lastColumn) {
  ...
  sheet.getRange(OFFSET_ROW + LABELS_TT_ROW.length, initColumn, 1, lastColumn - MAX_SESSION_NUM).setBackground(LABEL_COLOR_SESSION)
  sheet.getRange(OFFSET_ROW + LABELS_TT_ROW.length + 1, initColumn, 2, lastColumn - MAX_SESSION_NUM).setBackground(LABEL_COLOR_BASE)
  return lastColumn
}
...
function setSheetRss(sheet){
  ...
  sheet.getRange(lastRow, initColumn, ttLabelRowLen)
  .setValues(LABELS_TT_ROW)
  .setHorizontalAlignment("center");
  lastRow += ttLabelRowLen

  const inputLabelRowLen = LABELS_INPUT_ROW_RSS.length
  sheet.getRange(lastRow, initColumn, inputLabelRowLen)
...
function setSheetStudent(sheet){
  ...
  LABELS_INPUT_ROW_STUDENT.forEach((_, idx) => {
    const cell = sheet.getRange(OFFSET_ROW + LABELS_TT_ROW.length + idx + 1, initColumn+1, 1, lastColumn - MAX_SESSION_NUM)
